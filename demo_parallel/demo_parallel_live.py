...
outs = par(delayed(_get_number)() for _ in range(n_numbers))
print(outs)
...
def _get_reproducible_number(seed):
    np.random.seed(seed)
    return _get_number()
...
par = Parallel(n_jobs=2)
outs = par(delayed(_get_reproducible_number)(seed) for seed in seeds)
print(outs)
...
# seeds = np.ones(n_sims, dtype=int)
...
sns.stripplot(data=ari_df, x="Method", y="ARI", jitter=0.45)
